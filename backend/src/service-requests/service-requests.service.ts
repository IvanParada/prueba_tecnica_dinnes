...
export class ServiceRequestsService {
    ...
    async update(
        ...
        if (this.hasCustomerChanges(dto)) {
            serviceRequest.customer =
                await this.resolveCustomerForUpdate(
                    serviceRequest.customer,
                    dto,
                );
        }

        this.serviceRequestRepository.merge(
            serviceRequest,
            {
                number:
                    dto.number !== undefined
    ...
    private async resolveCustomerForUpdate(
        ...
        const name =
            updateServiceRequestDto.customerName ??
            currentCustomer.name;

        const email =
            updateServiceRequestDto.customerEmail ??
            currentCustomer.email;

        const phone =
            updateServiceRequestDto.customerPhone ??
