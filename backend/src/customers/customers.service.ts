...
export class CustomersService {
    private readonly apiUrl = process.env.CUSTOMERS_API_URL || 'https://jsonplaceholder.typicode.com/users';
    private readonly apiTimeout = parseInt(process.env.CUSTOMER_API_TIMEOUT || '3000', 10);
    ...
    async lookupCustomerByEmail(email: string): Promise<CustomerLookup> {
        ...
        try {
            const { data } = await firstValueFrom(
                this.httpService.get<CustomerLookup[]>(
                    this.apiUrl,
                    {
